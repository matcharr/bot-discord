...
class Admin(commands.Cog):
    ...
    async def list_backups(self, ctx):
        ...
        backup_list = []
        for backup in sorted(backups, key=lambda x: x.stat().st_mtime, reverse=True)[:10]:
            size_kb = backup.stat().st_size / 1024
            modified = datetime.fromtimestamp(backup.stat().st_mtime)
            backup_list.append(f"`{backup.name}` ({size_kb:.1f}KB, {modified.strftime('%Y-%m-%d %H:%M')})")
    ...
    async def show_config(self, ctx):
        ...
        embed.add_field(
            name="Logging",
            value=f"Log Level: {config.log_level}",
            inline=True
        )
