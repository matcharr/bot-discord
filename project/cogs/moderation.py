...
class Moderation(commands.Cog):
    ...
    async def clear_warnings(self, ctx, member: discord.Member):
        ...
        if user_id in self.warnings:
            ...
            await ctx.send(f"✅ Cleared {warning_count} warning(s) for {member.mention}")
            await log_moderation_action("CLEAR_WARNINGS", ctx.author, member, f"Cleared {warning_count} warnings", ctx.guild)
        else:
