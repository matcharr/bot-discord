...
from .models import SecureWarning, ModerationLog
import logging
...
class DatabaseCleanup:
    ...
    def hard_delete_old_soft_deleted(self, days_old: int = 90):
        ...
        try:
            ...
            old_warnings = self.db.query(SecureWarning).filter(
                and_(
            ...
            if count > 0:
                ...
                self.db.commit()
                logger.info(f"Hard deleted {count} old warnings (older than {days_old} days)")
            else:
    ...
    def cleanup_old_logs(self, days_old: int = 730):
        ...
        try:
            deleted_count = self.db.query(ModerationLog).filter(
                ModerationLog.created_at < cutoff_date
            ).delete()
            
            self.db.commit()
            logger.info(f"Deleted {deleted_count} old moderation logs (older than {days_old} days)")
    ...
    def get_cleanup_stats(self) -> dict:
        ...
        try:
            ...
            soft_deleted_30d = self.db.query(SecureWarning).filter(
                and_(
                    SecureWarning.is_deleted == True,
            ...
            soft_deleted_180d = self.db.query(SecureWarning).filter(
                and_(
                    SecureWarning.is_deleted == True,
            ...
            # Count old logs
            old_logs_365d = self.db.query(ModerationLog).filter(
                ModerationLog.created_at < now - timedelta(days=365)
            ).count()
