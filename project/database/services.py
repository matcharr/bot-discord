...
from .models import SecureWarning, ModerationLog, GDPRRequest
...
class WarningService:
    ...
    def get_user_warnings(
        ...
        try:
            ...
            logger.info(f"Retrieved {len(warnings)} warnings for user "
    ...
    def get_warning_count(self, guild_id: str, user_id: str) -> int:
        ...
        try:
            ...
            count = self.db.query(SecureWarning).filter(
                and_(
                    SecureWarning.guild_id_hash == guild_hash,
                    SecureWarning.user_id_hash == user_hash,
    ...
    def delete_warning(self, warning_id: int) -> bool:
        ...
        try:
            warning = self.db.query(SecureWarning).filter(
    ...
    def export_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get all warnings
            warnings = self.db.query(SecureWarning).filter(
                SecureWarning.user_id_hash == user_hash
            ).all()
            
            # Get all moderation logs
            logs = self.db.query(ModerationLog).filter(
            ...
            logger.info(f"Data exported for user {security_manager.anonymize_for_logs(user_id)}")
            return export_data
    ...
    def delete_user_data(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Data deleted for user {security_manager.anonymize_for_logs(user_id)}")
            return True
