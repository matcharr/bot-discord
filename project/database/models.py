...
class SecureWarning(Base):
    ...
    def create_warning(
        ...
        logger.info(f"Creating warning for user {security_manager.anonymize_for_logs(user_id)} "
...
class ModerationLog(Base):
    ...
    # Action details
    action_type = Column(String(50), nullable=False, index=True)  # warn, kick, ban, etc.
    ...
    def validate_action_type(self, key, value):
        ...
        allowed_actions = {'warn', 'kick', 'ban', 'unban', 'mute', 'unmute', 'timeout', 'purge'}
        if value not in allowed_actions:
            raise ValueError(f"Invalid action type: {value}. Must be one of {allowed_actions}")
        return value
    ...
    def create_log(
        ...
        logger.info(f"Logging {action_type} action by moderator "
        ...
        return cls(
            ...
            context_encrypted=security_manager.encrypt_text(context) if context else None,
