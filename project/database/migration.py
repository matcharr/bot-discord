...
class WarningMigration:
    ...
    def __init__(self):
        ...
        self.backup_file = self.data_dir / f"warnings_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def migrate_warnings(self, guild_id: str, moderator_id: str = "000000000000000000") -> dict:
        """
                    ...
                    for warning_reason in warnings_list:
                        ...
                        try:
                            ...
                            warning = service.add_warning(
                                ...
                                reason=warning_reason or "Migrated warning - no reason provided"
                            )
                            
                            stats["migrated_warnings"] += 1
                            logger.debug(f"Migrated warning {warning.id} for user {user_id}")
                            
                        except Exception as e:
                            stats["failed_warnings"] += 1
                            error_msg = f"Failed to migrate warning for user {user_id}: {e}"
                            stats["errors"].append(error_msg)
                            logger.error(error_msg)
                
                logger.info(f"Migration completed: {stats['migrated_warnings']}/{stats['total_warnings']} warnings migrated")
    ...
    def verify_migration(self, guild_id: str) -> dict:
        ...
        try:
            # Count JSON warnings
            json_warnings = self.load_json_warnings()
            json_count = sum(len(warnings) for warnings in json_warnings.values() if isinstance(warnings, list))
            ...
            try:
                ...
                db_count = service.db.query(SecureWarning).filter(
            ...
            verification["match"] = verification["database_warnings"] >= verification["json_warnings"]
...
if __name__ == "__main__":
    ...
    moderator_id = input("Enter default moderator ID (or press Enter for system): ").strip()
